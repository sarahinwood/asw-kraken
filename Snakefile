...
rule filter_baculovirus_reads:
	...
	shell:
		...
		'in2={input.r2} '
		'include=t '
		'names={input.bac_read_names} '
		'out={output.bac_reads_r1} '
		'out2={output.bac_reads_r2} '
		'&> {log}'
...
rule read_names:
	...
	threads:
		20
	log:
		'output/logs/r/fix_read_names.log'
	script:
		'src/kraken_bac_read_names.R'

rule grep_baculovirus:
	input:
		kraken_report = 'output/kraken_all/kraken_out.txt'
	output:
		kraken_bac_res = 'output/bac_reads/kraken_bac_res.txt'
	log:
		'output/logs/grep_baculovirus.log'
	threads:
		20
	shell:
		'egrep -i "baculoviridae|alphabaculovirus|nucleopolyhedrovirus|Condylorrhiza vestigialis MNPV|Pseudoplusia includens SNPV IE|Betabaculovirus|granulovirus" {input.kraken_report} > {output.kraken_bac_res} '
...
rule kraken:
	...
	output:
		out = 'output/kraken_all/kraken_out.txt',
		report = 'output/kraken_all/kraken_report.txt'
	log:
		'output/logs/kraken_all.log'
	threads:
		20
	singularity:
		kraken_container
